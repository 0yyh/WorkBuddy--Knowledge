...
| 技术路线 | React 18 + Vite 5 + TypeScript 5 + MUI 6 + Tailwind 3 + Capacitor 7 |
...
| 框架 | React | ^18.3.1 | 用户已定；**不上 19**（MUI 6 与 Capacitor 生态兼容性尚未稳定） | React 19（MUI 6.1 对 19 支持不完整） |
| 构建 | Vite | ^5.4.10 | 快；`base:'./'` + HashRouter 可产出 capacitor 可用的相对路径产物 | Vite 6/7（Tailwind 3 + MUI 6 插件链验证不足） |
| UI 组件 | MUI | ^6.1.6 | Drawer/SwipeableDrawer/主题系统/安全区组件开箱即用 | antd-mobile（桌面端弱）、自研（成本高） |
| 样式 | Tailwind CSS | ^3.4.14 | 阅读区精细排版（字号/行距/页宽/段距）用工具类最省 | Tailwind v4（需新构建链，v3 更稳）；纯 CSS Modules（变量驱动的主题不好做） |
| CSS-in-JS | Emotion | ^11.13.3 | MUI 6 的硬性依赖 | styled-components（MUI 不支持） |
...
| 项 | 预算 |
| --- | --- |
| 应用 JS（gzip 后） | ≤ 900KB（React+MUI+Tailwind+markdown-it+fflate+zustand+router） |
...
export interface IndexManifest {
  ...
  entryShards: string[];                   // ["a","b",...] 实际存在的分片
...
// .index/entries/a.json —— 短 key 以压缩体积（解码函数在 core 内）
...
graph TB
    ...
    subgraph L4["④ UI 层 UI Layer（React + MUI + Tailwind）"]
        direction TB
...
| `web/src/domain` | `core` + `web/src/platform` | ❌ 不得直接 `fetch`、不得用 `localStorage`、不得 import MUI |
| `web/src/ui` | `domain` + `platform` + MUI/Tailwind | ❌ 不得直接读 VFS |
...
| 任务(最小版) | 必留 | 砍掉 |
| --- | --- | --- |
| T01mini | workspace/vite/tsconfig/Tailwind/壳/router/taxonomy 雏形/规范冻结 | 审校 admin 脚手架、PWA |
...
- **轴 B 词条元数据分片（entries/*.json）**：保留 26 字母片，**不急于上 676**；15k 词条全量 ~8–14MB raw ÷26 ≈ 0.4–0.6MB/片，LRU(8) 够；仅单片 raw >1.5–2MB 时启"首字母+第二字母 676"。
